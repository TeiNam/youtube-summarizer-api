...
# 자막 충분성 게이트(MIN_SUBTITLE_CHARS=100)를 통과하는 길이의 더미 자막
SUFFICIENT_SUBTITLE = "This is a sufficiently long subtitle text. " * 5
...
class TestPipelineSubtitleSuccess:
    ...
    async def test_full_pipeline_with_subtitle(
        ...
        mock_translate.assert_called_once_with(SUFFICIENT_SUBTITLE, "ko")
...
class TestPipelineExtractionFailure:
    ...
    async def test_both_extraction_methods_fail(
        ...
        """자막과 음성 인식 모두 실패하면 상태가 failed이고 사용자용 메시지를 보여야 한다."""
        from app.services.pipeline import EXTRACTION_FAILED_MESSAGE
        ...
        assert task["error"] == EXTRACTION_FAILED_MESSAGE
        # 내부 에러 메시지는 사용자에게 노출되지 않아야 함
        assert "오디오 다운로드 실패" not in task["error"]
